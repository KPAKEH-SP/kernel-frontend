...
<template>
    ...
    <div v-if="openedFriendsList" class="list">
        <div v-for="friend in friends" :key="friend.id">
            <FriendCard v-if="friend.status == 'ACCEPTED'" :username="friend.firendUsername">
                <PhChatCircleDots class="blue-icon" :size="30" @click="emit('openChat', friend.firendUsername)"/>
                <PhTrash class="red-icon" :size="30" @click="emit('removeFriend', friend.firendUsername)"/>
            </FriendCard>
        </div>
    </div>
    ...
    <div v-if="openedRequestsList" class="list">
        <div v-for="friend in friends" :key="friend.id">
            ...
            <FriendCard v-if="friend.status == 'PENDING'" :username="friend.firendUsername">
                <PhCheck v-if="friend.pendingFrom != username" @click="emit('accept', friend.firendUsername)" class="blue-icon" :size="30"/>
                <PhSpinnerGap class="loading" v-else/>
                <PhX @click="emit('removeFriend', friend.firendUsername)" class="red-icon" :size="30"/>
            </FriendCard>
        </div>
    </div>
</template>
